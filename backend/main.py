...
# policies just get harmlessly re-written, changed ones get refreshed.
DATA_PIPELINE_DIR = os.path.join(_ROOT, "data-pipeline")


def refresh_policy_data():
    logger.info("Scheduled refresh: re-running data pipeline...")
    try:
        for script in ("scrape_policies.py", "clean_and_chunk.py", "build_vector_db.py"):
            subprocess.run(["python", script], cwd=DATA_PIPELINE_DIR, check=True)
        logger.info("Scheduled refresh complete.")
    except subprocess.CalledProcessError as e:
        logger.error("Scheduled refresh failed: %s", e)


# NOTE ON FRESHNESS: this only re-fetches the CONTENT of policies already
# listed in data/policy_index.json - it will pick up wording changes to
# known policies, but it will NOT detect a brand-new policy being added
# to the Policy Library, or an existing one being marked expired/rescinded.
# That roster is a manual snapshot (see discover_policies.py's --file/--url
# options) and should be re-generated by hand periodically, ideally once
